...
def send_slack_message(notification_content):
    if slackWebhookUrl != "":
        try:
            requests.post(slackWebhookUrl, data=json.dumps(notification_content),
                          headers={'Content-Type': 'application/json'})
            logging.info("Message sent to Slack webhook: %s", notification_content['text'])
        except (requests.exceptions.Timeout, ConnectionError) as err:
            logging.error("%s", err)


def send_smtp_message(notification_content):
    if emailReceiver != "" and smtpServer != "":
        try:
            email_content: str = re.sub('[^ :A-Za-z0-9]+', '', notification_content)
            email_message = EmailMessage()
            email_message.set_content(email_content)
            email_message['Subject'] = 'Container Watchdog Alert notification'
            email_message['From'] = emailSender
            email_message['To'] = emailReceiver
            mail = smtplib.SMTP(smtpServer, 25, timeout=40)
            mail.send_message(email_message)
            logging.info("Email sent to %s with content: %s", emailReceiver, email_content)
            mail.quit()
        except Exception as err:
            logging.error("%s", err)
...
def restart_container(container_object):
    try:
        ...
        notificationContent['text'] = ("[Container watchdog]: has *_restarted_* container: [ *_{0}_* ] which had healthstatus: [ _{1}_ ] and state:"
                                       " [ _{2}_ ] on hostmachine [ _{3}_ ]".format(container_object.name, container_health_status, containerStatus, dockerHost))
        if container_object.short_id not in restartedContainers:
            restartedContainers.append(container_object.short_id)
    except Exception as err:
        logging.fatal("%s", err)
        notificationContent['text'] = ("[Container watchdog]: Docker daemon failed to restart container *{0}* on hostmachine *{1}*"
                                       " with error message: _{2}_".format(container_object.name, dockerHost, err))
...
def container_recovered(container_object):
    ...
    notificationContent['text'] = ("[Container watchdog]: Container: [ *_{0}_* ] has *recovered* with healthstatus: [ _{1}_ ] and state: [ _{2}_ ]"
                                   " on hostmachine [ _{3}_ ]".format(container_object.name, container_health_status, containerStatus, dockerHost))
    restartedContainers.remove(container_object.short_id)
...
while True:
    ...
    for container in ContainerList:
        ...
        if container.short_id in restartedContainers and container_health_status == 'healthy':
            container_recovered(container)
            send_slack_message(notificationContent)
            send_smtp_message(notificationContent['text'])
        ...
        # Add container to list of restarted containers.
            ...
            logging.error("Found container in unhealthy state! Container: %s has health status: %s and container status: %s",
                          container.name, container_health_status, containerStatus)
            restart_container(container)
            send_slack_message(notificationContent)
            send_smtp_message(notificationContent['text'])
            restartStatus = True
        logging.debug('%s - %s - %s', container.name, container_health_status, containerStatus)
